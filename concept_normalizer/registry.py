...
Some source concepts have no equivalent in the target vocabulary.  Where the
target is OMOP, the supported answer is to mint local concepts and add them to
CONCEPT so downstream tooling can process them:
  ...
  * CONCEPT_ANCESTOR  — generated from the ontology's own parent/child tree, so
                        ...
                        Without these rows a custom concept exists in the CDM but
                        is invisible to generated SQL, which expands concept sets
                        by descendant.
  * CONCEPT_RELATIONSHIP — 'Maps to' rows for concepts that DO have a standard
                        equivalent, so a consumer resolves them to the standard
                        concept and keeps the local one as the source concept.

Domain assignment, in priority order:
  1. inherited from a concept's own mapping to the target (the mapped concept's
     real domain)
  2. inherited from the nearest ancestor with an assigned domain
  3. DEFAULT_DOMAIN ("Observation") — correct for most social/behavioural data

Every concept records which rule decided its domain, so a wrong one is findable
and correctable later rather than silently baked in.  Inheritance is preferred
over per-concept guessing because it is consistent by construction: siblings land
in the same domain, which an LLM asked 660 separate times will not guarantee.

Re-runnable by design: source ontologies grow (curation promotes new terms), so
